...
export const useRuntimeStore = create<RuntimeState>((set) => ({
	...
	onToolResult: (toolCallId, _toolName, result, success) => {
		set((s) => ({
			toolRecords: s.toolRecords.map((t) => (t.toolCallId === toolCallId ? { ...t, result, success, isComplete: true } : t))
		}))
	},

	onToolCallEnd: (toolCallId, args) => {
		set((s) => ({
			toolRecords: s.toolRecords.map((t) => (t.toolCallId === toolCallId ? { ...t, arguments: args } : t))
		}))
	},
