# Runtime Event System 规格

## Purpose

定义 runtime 事件的两层架构：ProviderEvent（provider 流式事件）和 RuntimeEvent（包含 ProviderEvent + RuntimeLifecycleEvent 的顶层事件类型），作为 LLM provider 与 runtime 之间的通信契约。
