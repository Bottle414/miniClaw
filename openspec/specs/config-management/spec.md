...
管理运行时配置的创建、验证和注入，支持环境变量加载和默认值设置。

## Requirements

### Requirement: 配置工厂创建 Config 对象
