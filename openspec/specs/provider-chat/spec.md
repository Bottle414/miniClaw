...
Provider SHALL 提供 `chat(req: LLMRequest): Promise<LLMResponse>` 方法，接受统一 LLM 请求类型。拥有权威对话历史和记忆状态访问权的运行时代码应在调用 Provider 前构建面向模型的 `contextMessages`，并将 `contextMessages` 作为 `req.messages` 传递。
