...
系统应将 `messages` 保持为当前会话的权威完整对话历史，包括系统、用户、助手和工具消息。

#### 场景：用户消息被记录

- **WHEN** 用户提交输入
- **THEN** 系统应将用户消息追加到 `messages`
- **AND** 系统不应在 `messages` 中丢弃或摘要该消息

#### 场景：助手和工具消息被记录

- **WHEN** 模型返回助手消息或工具执行返回工具消息
- **THEN** 系统应将这些消息追加到 `messages`
- **AND** 追加的消息应在权威历史中保持可用

### 需求：模型调用使用上下文消息

系统应为模型调用构建独立的 `contextMessages` 列表，而非直接传递完整权威 `messages` 列表。
...
系统应提供会话记忆，用于会话作用域的保留上下文，可注入到面向模型的上下文中。
...
系统应提供工作记忆，用于临时任务或迭代上下文，可在上下文构建时使用。
...
上下文构建器应在生成 `contextMessages` 时支持显式的保留、丢弃、注入和摘要操作。
...
系统应包含简单摘要模块，可将选定消息转换为简洁的模型可读摘要，无需额外模型调用。
